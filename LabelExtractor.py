...
class LearnLinearFilter(Task):
    ...
    def execute(self):
        ...
        for item in test_dataset:
            item["representation"] = (
                linear_model(item["representation"].cuda()).cpu().detach()
            )
        ...
        logging.info(
            f"metrics on inference with trainsformed representations: {metrics}"
        )

        # save inferences
        fileName = (
            f"Cleaned_Inference_{self.model_name.split('/')[-1]}_l{self.layer}.json"
        )
...
class EvalLabelExtractor(Task):
    ...
    def execute(self):
        ...
        if (Path(self.job_path) / evalFileName).exists():
            logging.info(
                f"{evalFileName} already exixts in {self.job_path}, returing..."
            )
            return
        else:
